...
// TODO: Do we even care about the other representations in p256_func? Could just hardcode and remove
...
template P256PointOnLine() {
    ...
    component on_line = PointOnLine(43, 6, params[2]);
    for (var i = 0; i < 6; i++) {
        on_line.in[0][0][i] <== x1[i];
        on_line.in[0][1][i] <== y1[i];
        on_line.in[1][0][i] <== x2[i];
        on_line.in[1][1][i] <== y2[i];
        on_line.in[2][0][i] <== x3[i];
        on_line.in[2][1][i] <== y3[i];
    }
...
template P256PointOnTangent() {
    signal input x1[6]; // each register is 64-bits 
    signal input y1[6];
    signal input x3[6];
    signal input y3[6];
    ...
    component on_tangent = PointOnTangent(43, 6, params[0], params[2]);
    for (var i = 0; i < 6; i++) {
        on_tangent.in[0][0][i] <== x1[i];
        on_tangent.in[0][1][i] <== y1[i];
        on_tangent.in[1][0][i] <== x3[i];
        on_tangent.in[1][1][i] <== y3[i];
    }
...
template P256PointOnCurve() {
    ...
    component on_curve = PointOnCurve(43, 6, params[0], params[1], params[2]);
    for (var i = 0; i < 6; i++) {
        on_curve.in[0][0] <== x[i];
        on_curve.in[0][1] <== y[i];
    }
...
template P256AddUnequal(n, k) {
    ...
    // [A,B,P,N]
    var params[4][6] = get_p256_params();
    component adder = EllipticCurveAddUnequal(43, 6, params[2]);
    for (var i = 0; i < 6; i++) {
        adder.a[0][i] <== a[0][i];
        adder.a[1][i] <== a[1][i];
        adder.b[0][i] <== b[0][i];
...
template P256Double(n, k) {
    ...
    // [A,B,P,N]
    var params[4][6] = get_p256_params();
    component doubler = EllipticCurveDouble(43, k, params[0], params[1], params[2]);
    for (var i = 0; i < 6; i++) {
